...
void (*old_WeaponUpdate)(...);
void new_WeaponUpdate(void* instance) {

    if (instance != nullptr) {
		
		
		if(damagee){
			*(int *) ((uint64_t) instance + damage) = 600;}
			
			
		if(bulletLeft){
				*(int *) ((uint64_t) instance + bulletsLeft) = 99;
				*(int *) ((uint64_t) instance + ammo) = 99;}
				
			
		
				
			if(fireRatee){
			*(float *) ((uint64_t) instance + fireRate) = 0.01f;}
